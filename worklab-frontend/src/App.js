...
const App = () => {
  ...
  const handleAddPaciente = async () => {
    if (!novoPaciente.nomeCompleto || !novoPaciente.sexo || !novoPaciente.email || !novoPaciente.celular) {
  ...
  const handleAddExame = async () => {
    if (!novoExame.codigo || !novoExame.descricao || !novoExame.valor) {
  ...
  const handleVincularExame = async () => {
    if (!pacienteSelecionado || !codigoExame) {
      alert('Selecione um paciente e insira o código do exame.');
      return;
    }
